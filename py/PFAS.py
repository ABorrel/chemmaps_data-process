...
class PFAS:
    ...
    def __init__(self, p_listChem, name_map, istart, iend, p_dir_Desc, p_dir_out):
    ...
    def prep_chem(self):
        ...
        for d_chem in l_chem:
            filout.write("%s\t%s\t%s\t%s\t%s\n" % (d_chem["DTXSID"], d_chem["SMILES"], d_chem["smiles_clean"], d_chem["inchikey"], d_chem["PREFERRED NAME"]))
        filout.close() 
    ...
    def compute_pushDB_Desc(self):
        ...
        cmd_inch = "select inchikey from chemical_description where map_name = 'pfas'"
        ...
        for d_chem_to_compute in l_chem_to_compute:
            ...
            if self.c_CompDesc.err == 0:
                ...
                self.c_DB.addElement("chemical_description", ["inchikey", "desc_1d2d", "desc_3D", "map_name"], [inchikey, w1D2D, w3D, "pfas"])
